...
function RadiusCard({ name, value, isDarkMode }: RadiusCardProps) {
  const [copied, setCopied] = useState(false);

  const handleCopy = () => {
    navigator.clipboard.writeText(value >= 9999 ? '9999px' : `${value}px`);
    setCopied(true);
    setTimeout(() => setCopied(false), 1500);
  };

  // Cap display radius at 32 for visual purposes (but show full circle for large values)
  const displayRadius = value >= 9999 ? '50%' : Math.min(value, 32);
    ...
    >
      ...
      <div style={infoStyle}>
        ...
        <div style={valueStyle}>
          {value >= 9999 ? 'full (circle)' : `${value}px`}
        </div>
      </div>
    </div>
  );
...
export function RadiusTokensDisplay({ radius, isDarkMode = false }: RadiusTokensDisplayProps) {
  ...
  return (
    <div style={sectionStyle}>
      ...
      <p style={descriptionStyle}>
        Border radius tokens for rounded corners. Click to copy values.
      </p>

      <div style={gridStyle}>
        {radius.tokens.map((token, index) => {
